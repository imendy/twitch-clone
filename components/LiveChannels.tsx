import React from 'react';
import LiveChannelsItems from './LiveChannelsItems'
import Live1 from '../public/live1.jpg'
import Live2 from '../public/live2.jpg'
import Live3 from '../public/live3.jpg'
import Live4 from '../public/live4.jpg'
import Live5 from '../public/live5.jpg'
import Live6 from '../public/live6.jpg'
import Live7 from '../public/live7.jpg'
import Live8 from '../public/live8.jpg'
import Live9 from '../public/live9.jpg'
import Live10 from '../public/live10.jpg'


const LiveChannels = () => {
  return (
    <div id='live' className="mt-4 p-2 md:p-3">
      <h2 className="px-2 font-bold"> 
        <span className="text-[#9147ff]"> Live Channels </span>We think you'll love this
      </h2>

      {/* Channel Container */}

      <div className="grid min-[500px]:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 py-2">
        <LiveChannelsItems
          img={Live1}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/b3fc1f16-0818-4121-8711-c31b9469bc21-profile_image-50x50.png'
          title='CHARITY STREAM! Any support is ap...'
          user='Natt'
          game='Valorant'
        />
        <LiveChannelsItems
          img={Live2}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/958d17a9-0c82-4210-8376-7e3c00a006f5-profile_image-50x50.png'
          title='Deployable shield commin in clutch'
          user='50ZINT'
          game='Call of Duty: Warzone'
        />
        <LiveChannelsItems
          img={Live3}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/7ed5e0c6-0191-4eef-8328-4af6e4ea5318-profile_image-50x50.png'
          title='VALORANT WITH NIJI AND FRIENDS!...'
          user='baoo'
          game='Valorant'
        />
        
        <LiveChannelsItems
          img={Live4}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/jerma985-profile_image-447425e773e6fd5c-50x50.jpeg'
          title='Local Streamer stuck in a cave: sings...'
          user='Jerma985'
          game='One Hand Clapping'
        />
        <LiveChannelsItems
          img={Live5}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/bed3ea39-265b-4632-b866-598bb8c045dd-profile_image-50x50.png'
          title='BIG Mario Kart session'
          user='Bigpuffer'
          game='Mario Kart 8 Deluxe'
        />
        <LiveChannelsItems
          img={Live6}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/b3c347ed-1a7a-40a2-8bee-8a7c4426eb33-profile_image-50x50.png'
          title='Tripod Man Gains RP | Code: MFAM | @...'
          user='NICKMERCS'
          game='Apex Legends'
        />
        <LiveChannelsItems
          img={Live7}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/jerma985-profile_image-447425e773e6fd5c-50x50.jpeg'
          title='JESUS LOVES YOU!!!'
          user='coletteleclair'
          game='Just Chatting'
        />
        <LiveChannelsItems
          img={Live8}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/b3fc1f16-0818-4121-8711-c31b9469bc21-profile_image-50x50.png'
          title='Covert_Muffin teaching Star Wars...'
          user='JamesDoneQuick'
          game='Star Wars: Jedi Knight II - Jedi Outcast'
        />
        <LiveChannelsItems
          img={Live9}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/7ed5e0c6-0191-4eef-8328-4af6e4ea5318-profile_image-50x50.png'
          title='Grinding'
          user='shroud'
          game='The Cycle: Frontier'
        />
        <LiveChannelsItems
          img={Live10}
          profile_img='https://static-cdn.jtvnw.net/jtv_user_pictures/958d17a9-0c82-4210-8376-7e3c00a006f5-profile_image-50x50.png'
          title='GRILLIN / CHILLEN! Steak / Burgers...'
          user='KennyGoodloe'
          game='Just Chatting'
        />
      </div>
      
    </div>
  )
}

export default LiveChannels